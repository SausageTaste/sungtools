...
namespace sung {
    ...
    public:
        ...
        template <typename U>
        constexpr TVec3(const TVec4<U>& other) : elements_{other.x(), other.y(), other.z()} {}

        // Element-wise operations
        constexpr TVec3 operator+(const TVec3& rhs) const {
            return TVec3{this->x() + rhs.x(), this->y() + rhs.y(), this->z() + rhs.z()};
        }
        constexpr TVec3 operator-(const TVec3& rhs) const {
            return TVec3{this->x() - rhs.x(), this->y() - rhs.y(), this->z() - rhs.z()};
        }
        ...
        constexpr T len_sqr() const {
            return this->dot(*this);
        }

        T len() const {
            return std::sqrt(this->len_sqr());
        }
        ...
        T distance(const TVec3& rhs) const {
            return (*this - rhs).len();
        }
    ...
    public:
        ...
        constexpr TVec4(const TVec3<U>& v, V w)
            : elements_{static_cast<T>(v.x()), static_cast<T>(v.y()), static_cast<T>(v.z()), static_cast<T>(w)}
        {}

        template <typename U>
        constexpr TVec4(const TVec4<U>& other)
            : elements_{static_cast<T>(other.x()), static_cast<T>(other.y()), static_cast<T>(other.z()), static_cast<T>(other.w())}
        {}

        // Element-wise operations
        constexpr TVec4 operator+(const TVec4& rhs) const {
            return TVec4{this->x() + rhs.x(), this->y() + rhs.y(), this->z() + rhs.z(), this->w() + rhs.w()};
        }
        constexpr TVec4 operator-(const TVec4& rhs) const {
            return TVec4{this->x() - rhs.x(), this->y() - rhs.y(), this->z() - rhs.z(), this->w() - rhs.w()};
        }
        ...
        constexpr TVec4 operator*(T rhs) const {
            return TVec4{this->x() * rhs, this->y() * rhs, this->z() * rhs, this->w() * rhs};
        }
        constexpr TVec4 operator/(T rhs) const {
            return TVec4{this->x() / rhs, this->y() / rhs, this->z() / rhs, this->w() / rhs};
        }
        ...
        T distance(const TVec4& rhs) const {
            return (*this - rhs).len();
        }

        TVec4 normalize() const {
            const auto l = this->len();
            return TVec4{this->x() / l, this->y() / l, this->z() / l, this->w() / l};
        }
    ...
    public:
        ...
        constexpr T at(size_t row, size_t col) const { return elements_[row][col]; }
